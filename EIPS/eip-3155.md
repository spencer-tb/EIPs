...
| `stateGas`      | Hex-Number           | State gas reservoir left before executing this operation             |
| `stateGasCost`  | Hex-Number           | State gas cost of this operation                                    |
...
- The `stateGas` and `stateGasCost` fields MUST be present when executing under fork rules that include [EIP-8037](./eip-8037.md) (multidimensional gas). `stateGas` is the state gas reservoir remaining *before* the operation, and `stateGasCost` is the state gas charged by the operation. When state gas spills into regular gas (i.e. the reservoir is exhausted), `gasCost` reflects the spillover amount deducted from regular gas.
...
| `stateGasUsed` | Hex-Number | Total state gas consumed by the transaction           |

The `stateGasUsed` field MUST be present in the summary when executing under fork rules that include [EIP-8037](./eip-8037.md).
